...
class InteractiveSelector:
    ...
    def find_info_file(self, executable: Path) -> Optional[Path]:
        ...
        info_extensions = ['.nfo', '.txt', '.diz', '.readme']
                ...
                with open(info_file, 'r', encoding=encoding) as f:
                    ...
                    for i, line in enumerate(f):
                        if i >= max_lines:
                            ...
                            break
                        # Remove trailing whitespace and expand tabs to spaces
                        # Tabs can cause display issues in curses
    ...
    def draw_header(self, stdscr, width: int):
        ...
        if self.active_pane == 'preview':
            header = "PeelX - Preview Mode (Full Screen)"
            help_text = "↑/↓: Scroll | PgUp/PgDn: Fast Scroll | Home/End | ←: Back to List | ENTER: Run | q: Quit"
        else:
            header = "PeelX - Select Executable to Run"
            help_text = "↑/↓: Navigate | →: Full Preview | ENTER: Run | q: Quit"

        try:
            # Pad to full width to ensure proper display
            stdscr.addstr(0, 0, header[:width-1].ljust(width-1), curses.A_BOLD | curses.A_REVERSE)
            stdscr.addstr(1, 0, help_text[:width-1].ljust(width-1), curses.A_DIM)
        except curses.error:
            pass
    ...
    def draw_preview_fullscreen(self, stdscr, height: int, width: int):
        ...
        if info_file:
            ...
            for i in range(visible_height):
                ...
                # Use full width - reserve 1 char for scrollbar if needed
                max_line_width = width - 1 if len(content) > visible_height else width - 1

                # Truncate or pad line to exact width
                if len(line) > max_line_width:
                    display_line = line[:max_line_width]
                else:
                    # Pad short lines to fill width (helps visualize the actual display area)
                    display_line = line.ljust(max_line_width)
            ...
            # Draw scroll indicator if needed
            # Note: The scrollbar overwrites the last character of each line
            if len(content) > visible_height and width > 2:
                scroll_pos = int((self.preview_scroll / max_scroll) * (visible_height - 1)) if max_scroll > 0 else 0
                try:
                    for i in range(visible_height):
                        y_pos = 4 + i
                        # Draw scrollbar at the rightmost column
                        if i == scroll_pos:
    ...
    def draw_footer(self, stdscr, height: int, width: int):
        ...
        if 0 <= self.current_index < len(self.executables):
            ...
            try:
                # Pad to full width
                stdscr.addstr(height - 1, 0, footer[:width-1].ljust(width-1), curses.A_REVERSE)
            except curses.error:
                pass
    ...
    def run(self, stdscr) -> Optional[Path]:
        ...
        stdscr.clear()
        stdscr.refresh()  # Ensure screen is properly initialized
